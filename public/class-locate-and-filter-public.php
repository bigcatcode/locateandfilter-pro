...
class Locate_And_Filter_Public
{
    ...
    {
        wp_enqueue_style($this->plugin_name.'-all', plugin_dir_url(__FILE__).'css/locate-and-filter-public.css', [], $this->version, 'all');
        // leaflet css
        wp_enqueue_style($this->plugin_name.'-leaflet', plugin_dir_url(__FILE__).'js/leaflet-0.7.3/leaflet.css', [], $this->version, 'all');
        // leaflet-filters css
        wp_enqueue_style($this->plugin_name.'-leaflet-filters', plugin_dir_url(__FILE__).'js/leaflet-filters/leaflet-filters.css', [], $this->version, 'all');
        // leaflet markerCluster css
        wp_enqueue_style($this->plugin_name.'-leaflet-marker-cluster-default', plugin_dir_url(__FILE__).'js/leaflet.markercluster/MarkerCluster.Default.css', [], $this->version, 'all');
        wp_enqueue_style($this->plugin_name.'-leaflet-marker-cluster', plugin_dir_url(__FILE__).'js/leaflet.markercluster/MarkerCluster.css', [], $this->version, 'all');
        // Tokenize CSS
        wp_enqueue_style($this->plugin_name.'-tokenize', plugin_dir_url(__FILE__).'js/Tokenize-2.2.1/jquery.tokenize.css', [], $this->version, 'all');
        // Chosen CSS
        $load_chosen = unserialize(get_option('locate-anything-option-load-chosen'));
        if ($load_chosen) {
            wp_enqueue_style($this->plugin_name.'-chosen', plugin_dir_url(__FILE__).'js/chosen_v1.8.7/chosen.css', [], $this->version, 'all');
        }
        // Pretty-checkbox
        wp_enqueue_style($this->plugin_name.'-pretty-checkbox', plugin_dir_url(__FILE__).'js/pretty-checkbox/pretty-checkbox.min.css', [], $this->version, 'all');
        // leaflet Google automplete CSS
        wp_enqueue_style($this->plugin_name.'-googleauto', plugin_dir_url(__FILE__).'js/leaflet-google-autocomplete/css/leaflet-google-autocomplete.css', [], $this->version, 'all');
        // Ionicons
        wp_enqueue_style($this->plugin_name.'-ionicons', 'https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css', [], $this->version, 'all');
        // Awesome markers
        wp_enqueue_style($this->plugin_name.'-awesomemarkers', plugin_dir_url(__FILE__).'js/leaflet.awesome-markers-2.0/leaflet.awesome-markers.css', [], $this->version, 'all');
        wp_enqueue_style('jquery-ui-css', plugin_dir_url(__FILE__).'js/jquery-theme/jquery-ui.css', [], $this->version, 'all');
    }
    ...
    public function enqueue_scripts()
    {
        ...
        wp_enqueue_script($this->plugin_name.'-all', plugin_dir_url(__FILE__).'js/locate-and-filter-public.js', [
            'jquery',
        ], $this->version, false);
        ...
        if (array_search('google', $loadjs) !== false) {
            wp_enqueue_script($this->plugin_name.'-googleAPI', 'https://maps.googleapis.com/maps/api/js?v=3.exp&key='.$gmaps_key.'&libraries=places&language='.unserialize(get_option('locate-anything-option-map-language')), [
                $this->plugin_name.'-leaflet-filters',
            ], $this->version, false);
            // Google Tiles
            wp_enqueue_script($this->plugin_name.'-googleTiles', plugin_dir_url(__FILE__).'js/leaflet-plugins-master/layer/tile/Google.js', [
                $this->plugin_name.'-leaflet-filters',
            ], $this->version, false);
        }
        ...
        if (array_search('yandex', $loadjs) !== false) {
            wp_enqueue_script($this->plugin_name.'-yandexAPI', 'http://api-maps.yandex.ru/2.0/?load=package.map&lang='.unserialize(get_option('locate-anything-option-map-language')), [
                $this->plugin_name.'-leaflet-filters',
            ], $this->version, false);
            wp_enqueue_script($this->plugin_name.'-yandexTiles', plugin_dir_url(__FILE__).'js/leaflet-plugins-master/layer/tile/Yandex.js', [
                $this->plugin_name.'-leaflet-filters',
            ], $this->version, false);
        }
        ...
        if (array_search('bing', $loadjs) !== false) {
            wp_enqueue_script($this->plugin_name.'-bingTiles', plugin_dir_url(__FILE__).'js/leaflet-plugins-master/layer/tile/Bing.js', [
                $this->plugin_name.'-leaflet-filters',
            ], $this->version, false);
        }

        // leaflet JS
        wp_enqueue_script($this->plugin_name.'-leaflet', plugin_dir_url(__FILE__).'js/leaflet-0.7.3/leaflet.js', [
            'jquery',
        ], $this->version, false);
        // leaflet-filters JS
        wp_enqueue_script($this->plugin_name.'-leaflet-filters', plugin_dir_url(__FILE__).'js/leaflet-filters/leaflet-filters.js', [
            $this->plugin_name.'-leaflet',
        ], $this->version, false);
        // leaflet markerCluster JS
        wp_enqueue_script($this->plugin_name.'-leaflet-marker-cluster', plugin_dir_url(__FILE__).'js/leaflet.markercluster/leaflet.markercluster-src-min.js', [
            'jquery',
        ], $this->version, false);
        // Tokenize JS
        wp_enqueue_script($this->plugin_name.'-tokenize', plugin_dir_url(__FILE__).'js/Tokenize-2.2.1/jquery.tokenize.js', [
            'jquery',
        ], $this->version, false);
        ...
        $load_chosen = unserialize(get_option('locate-anything-option-load-chosen'));
        if ($load_chosen) {
            wp_enqueue_script($this->plugin_name.'-chosen', plugin_dir_url(__FILE__).'js/chosen_v1.8.7/chosen.jquery.min.js', [
                'jquery',
            ], $this->version, false);
        }
        // jQuery Array JS
        wp_enqueue_script($this->plugin_name.'-arrayUtilities', plugin_dir_url(__FILE__).'js/jquery.arrayUtilities.min.js', [
            'jquery',
        ], $this->version, false);
        // leaflet Google automplete JS
        wp_enqueue_script($this->plugin_name.'-googleautojs', plugin_dir_url(__FILE__).'js/leaflet-google-autocomplete/js/leaflet-google-autocomplete.js', [
            $this->plugin_name.'-leaflet-filters',
        ], $this->version, false);
        // Awesome markers
        wp_enqueue_script($this->plugin_name.'-awesomemarkersjs', plugin_dir_url(__FILE__).'js/leaflet.awesome-markers-2.0/leaflet.awesome-markers.min.js', [
            $this->plugin_name.'-leaflet-filters',
        ], $this->version, false);
        //leaflet-omnivore.min.js
        wp_enqueue_script($this->plugin_name.'-omnivorejs', plugin_dir_url(__FILE__).'js/leaflet-omnivore/leaflet-omnivore.min.js', [
            $this->plugin_name.'-leaflet-filters',
        ], $this->version, false);
        wp_localize_script($this->plugin_name.'-leaflet-marker-cluster', 'ClusterOptions', [
    ...
    public static function check_license_key($type_license)
    {
        ...
        if (substr($license_key, 0, 2) === 'CC') {
            return true;
        }

        if (hash('sha256', site_url().$seed) === $license_key) {
            return true;
        } else {
            return false;
        }
    ...
    public static function generateMapJS($map_id, $map_container)
    {
		...
		var current_map;
					...
					var params = {
						...
						<?php if ($params['style-hue']) {
                                echo '"style-hue":"'.$params['style-hue'].'",';
                            } ?>
					...
					eval("var "+map_instance+"=new leaflet_filters_class(params);");				 
					 ...
					 <?php if (has_filter('locate_anything_beforeCreateMap')) {
                                echo apply_filters('locate_anything_beforeCreateMap', $map_id);
                            } ?>
					 	/* Initialize Map  */	
					eval(map_instance).createMap();
					/*   Register filters, property_name is the name of the property as shown in the JSON datas  */
					var custom_filters= [<?php if (is_array($filters)) {
                                foreach ($filters as $filter) {
                                    echo '{"property_name":"'.$filter.'","html_id" : "#'.$filter.'-'.$map_id.'"},';
                                }
                            } ?>];
					...
					var createEverything_<?php echo $map_id?> = function(result){	
						...
						setTimeout(function(){						
							...
							<?php if (has_filter('locate_anything_afterGenerateJS')) {
                                echo apply_filters('locate_anything_afterGenerateJS', $map_id);
                            } ?>
							},250);
					}
    ...
    public static function defineMarkerIcon($post_params)
    {
        ...
        return ['id'=>$custom_marker, 'marker'=>$marker];
    }
    ...
    public static function defineDefaultMarker($params)
    {
        ...
        return ['id'=>$default_marker_id, 'marker'=>$marker];
    }
     ...
     *
     * @param int  $map_id map ID
     * @param bool $output determines if the function outputs the cache file content or runs silently
     *
     * @return void
     */
    ...
    public static function generateJSON($params, $output = false)
    {
        ...
        if (!isset($params['locate-anything-source'],$params['locate-anything-googleplaces'], $params['locate-anything-marker-type'],$params['locate-anything-usergeolocation-zoom'])) {
            exit(_e('Preview not available yet.', 'locate-and-filter'));
        }
        ...
        if (is_array($taxonomies)) {
            foreach ($taxonomies as $taxonomy) {
                ...
                if (!is_array($allowed)) {
                    $params['locate-anything-allowed-filters-value-'.$taxonomy] = unserialize($allowed);
                }
            }
        }
        ...
        if (Locate_And_Filter_Tools::getPostType($post_type) !== false) {
            ...
            if (is_array($taxonomies) && $post_type !== 'user') {
                foreach ($taxonomies as $taxonomy) {
                    $allowed = $params['locate-anything-allowed-filters-value-'.$taxonomy];
                    if (is_array($allowed)) {
                        $taxo_args[$taxonomy] = $allowed;
                    }
                }
            }
        ...
        $basic_fields = ['id', 'title', 'tooltip_template', 'lat', 'lng', 'street', 'streetnum', 'city', 'country', 'state', 'zip', 'custom_marker', 'css_class'];
        ...
        foreach ($posts as $post) {
            ...
            if ($preset2 !== false && !is_null($preset2) && $preset2 !== 'default') {
                $css_preset = $preset2;
            } else {
                $css_preset = $preset;
            }
            ...
            if ($css_preset == 'nice-tooltips') {
                ...
                if (strlen($maxheight2) > 2) {
                    $css_maxheight = $maxheight2;
                } else {
                    $css_maxheight = $maxheight;
                }
            ...
            foreach ($add as $tag=>$val) {
                if (array_search($tag, $tags_used) === false) {
                    unset($add[$tag]);
                }
            }
            ...
            if ($post_type !== 'user') {
                ...
                if (is_array($taxonomies) && is_array($filters)) {
                    $filter_taxonomies = array_unique(array_merge($filters, $taxonomies));
                } elseif (is_array($filters)) {
                    $filter_taxonomies = $filters;
                } elseif (is_array($taxonomies)) {
                    $filter_taxonomies = $taxonomies;
                }
            }
            if (is_array($filter_taxonomies)) {
                foreach ($filter_taxonomies as $taxonomy) {
                    if ($taxonomy != $post_type) {
                        ...
                        $arr_terms = [];
                        if ($taxonomies === false || array_search($taxonomy, $taxonomies) === false) {
                            $allowed = false;
                        } else {
                            $allowed = $params['locate-anything-allowed-filters-value-'.$taxonomy];
                        }
