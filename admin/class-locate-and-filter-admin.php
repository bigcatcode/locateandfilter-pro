...
class LocateAndFilterAdmin
{
     ...
     *             The GoogleMaps key.
     */
    public static $Gmaps_API_key = 'AIzaSyC0lZ7MbGfowxNTZva7fAyeTJ18dAWMUp0';
    public static $Bing_API_key = 'An5y6EfmR_Tmk0nMXSWFQD7JZ2sc5Zv793InR2eKkVmMpwxfa_XOzlEmIEPXIol9';
    ...
    public static function getGmapsAPIKey()
    {
        ...
        if ($key === false || empty($key)) {
            $key = Locate_And_Filter_Admin::$Gmaps_API_key;
        }
    ...
    public static function getBingAPIKey()
    {
        ...
        if ($key === false || empty($key)) {
            $key = Locate_And_Filter_Admin::$Bing_API_key;
        }
    ...
    public function enqueueScripts()
    {
        ...
        if (strpos($screen->base, 'user') === false && !in_array($screen->post_type, $allowed_post_types)) {
            return;
        }
    ...
    public function addPostMetaBoxes()
    {
        ...
        if (is_array($selected_post_types)) {
            $selected_post_types[] = 'locateanythingmarker';
        } else {
            $selected_post_types = [
    ...
    public function saveMetaboxData($post_id, $post)
    {
        ...
        if (isset($_POST['locate-anything-filters'])) {
            ...
            $locateanything_custom_field_for_tax_elment = [];
            $locateanything_custom_field_for_tax = get_option('locateanything_custom_field_for_tax');

            //status checkbox
            $locateanything_custom_field_for_tax_elment_checkbox = [];
     ...
     *
     * @param bool $post_type [description]
     *
        ...
        if ($post_type !== false) {
            foreach ($additional_field_list as $key => $field) {
                if ($field['post_type'] !== $post_type) {
                    unset($additional_field_list[$key]);
                }
            }
        }
    ...
    public function laGettaxonomyterms()
    {
        ...
        if ($terms) {
            foreach ($terms as $in => $term) {
                if (is_array($selected) && array_search($term->term_id, $selected) !== false) {
                    $terms[$in]->selected = 1;
                } else {
                    $terms[$in]->selected = 0;
                }
