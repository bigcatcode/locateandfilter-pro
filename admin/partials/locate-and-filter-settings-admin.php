...
<h2 class="nav-tab-wrapper">
    <?php
      $tabs = [__('Default settings', 'locate-anything')];
...
<option <?php if (unserialize(get_option('locate-anything-option-map-language')) == $locale) {
        echo 'selected';
...
<?php
            ...
            $loadjs = ['google', 'bing', 'yandex', 'none'];
            ...
            foreach ($loadjs as $loadjs_) {
                ...
                if (array_search($loadjs_, $selected_items) !== false) {
                    echo ' selected ';
                }
                echo '>'.$loadjs_.'</option>';
            }
...
<?php
            ...
            foreach ($post_types as $post_type) {
                if ($post_type->name == 'locateandfiltermap') {
                    continue;
                }
                echo '<option value="'.$post_type->name.'"';
                if (array_search($post_type->name, $selected_items) !== false) {
                    echo ' selected ';
                }
...
<li> <?php _e('Enable cache', 'locate-anything')?> : <input type="radio" name="locate-anything-option-enable-cache" value="1" <?php if (unserialize(get_option('locate-anything-option-enable-cache')) == 1) {
                                        echo 'checked';
                                    } ?> > <?php _e('yes', 'locate-anything')?> <input type="radio" <?php if (unserialize(get_option('locate-anything-option-enable-cache')) == 0) {
                                        echo 'checked';
...
<li> <input type="radio" name="locate-anything-option-load-chosen" value="1" <?php if (unserialize(get_option('locate-anything-option-load-chosen')) == 1) {
                                        echo 'checked';
                                    } ?> > <?php _e('yes', 'locate-anything')?> <input type="radio" <?php if (unserialize(get_option('locate-anything-option-load-chosen')) == 0) {
                                        echo 'checked';
