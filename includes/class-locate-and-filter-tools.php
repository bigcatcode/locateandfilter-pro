...
class Locate_And_Filter_Tools
{
    ...
    public static function getPostType($post_type_name)
    {
        ...
        if (array_search($post_type_name, $post_types) === false) {
            return false;
        } else {
            return true;
        }
    ...
    public static function getSelectForType($type, $name, $tokenize = true, $maxElement = 99999)
    {
        ...
        if ($posts) {
            ...
            foreach ($posts as $k=>$post) {
                ...
                if ($type == 'tealog') {
                    $str .= ' &nbsp;('.get_field('street_address', $post->ID).','.ucfirst(get_field('city', $post->ID)).','.ucfirst(get_field('country', $post->ID)).')</option>';
                }
                $li[] = $str;
            }
    ...
    public static function getCheckboxesForTaxonomy($taxonomy, $name, $allowed, $icon, $pretty, $categoryfilter = '')
    {
        ...
        if ($terms) {
            ...
            foreach ($terms as $k=>$term) {
                ...
                if ($locateanything_checkbox_status == 'unchecked') {
                    $status = '';
                } else {
                    $status = 'checked';
                }
                if ($categoryfilter != $term->term_id) {
                    $status = '';
                } else {
                    $status = 'checked';
                }
    ...
    public static function findSomething($post_type, $taxonomies, $taxo_args = [], $query_args = [], $relation = 'OR')
    {
        ...
        if (is_array($taxonomies)) {
            foreach ($taxonomies as $taxonomy) {
                if (isset($taxo_args[$taxonomy]) && !empty($taxo_args[$taxonomy])) {
                    $myquery['tax_query'][] = ['taxonomy' => $taxonomy, 'terms' => $taxo_args[$taxonomy], 'field' => 'term_id'];
                }
            }
        }
