...
class Locate_And_Filter_Addon_Helper
{
    ...
    {
        add_filter('locate_anything_add_custom_filters', function ($filter_html, $map_id, $filters) use ($arr_filters) {
            foreach ($filters as $filter_fieldname) {
                $index = in_array($filter_fieldname, array_keys($arr_filters));
                if ($index) {
    ...
    public static function create_filter_choice($filter_field, $title, $map_id)
    {
        ...
        $f .= Locate_And_Filter_Tools::Array2Select([
            //"radio"=> "Radio buttons",
            'checkbox'       => 'Checkboxes',
            'select'         => 'Dropdown',
        ...
        if ($filter) {
            ...
            foreach ($values as $value) {
                switch ($selector) {
                    ...
                    case 'radio':
                    if (!empty($value)) {
                        $f .= "<label><input type='$selector'  name='".$filter.'-'.$map_id."[]' id='".$filter.'-'.$map_id."' value='".esc_attr(stripslashes($value))."'>".stripslashes($value).'</label>';
                    }
                        break;
